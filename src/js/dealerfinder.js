...
lang['en'] = {
	'name': '',
	'searchResultName': '<strong> Sakos</strong>',
	'phone': 'Phone',
	'address': 'Address',
	'zoom': 'Zoom',
	'direction': 'Direction',
					...
					success: function (msg) {
						...
						if (results.length > 0) {
							...
							jQuery.each(results, function (k, v) {
								...
								if (number > 9) {
									$("<li id='l_" + marker['__gm_id'] + "' class='item-contact'" + "data-iframe='" + v.getAttribute("des") + "'>")
										.html('<div class="block-title">' + v.getAttribute("name") +
											'</div><div class="wrap-content"><em class="lnr lnr-map-marker"></em>' + '<span>' +
											v.getAttribute("address") + '</span>' + '<div class="wrap-content"><em class="lnr lnr-phone-handset"></em><span>' +
											v.getAttribute("phone") + '</span></div></li>')
										.click(function () {
...
function sort_distance() {
	...
	items.sort(function (a, b) {
        ...
        */
		...
		if ((Math.round(parseFloat(keyA) * 100) / 100) < (Math.round(parseFloat(keyB) * 100) / 100)) {
			return -1;
		}
		if ((Math.round(parseFloat(keyA) * 100) / 100) > (Math.round(parseFloat(keyB) * 100) / 100)) {
			return 1;
