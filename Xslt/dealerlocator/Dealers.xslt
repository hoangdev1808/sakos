<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" version="1.0" exclude-result-prefixes="msxsl">
        ...
        <xsl:choose>
            <xsl:when test="count(/DealerList/DealerGroup) > 0">
                <div class="intro-mangluoi">
                    <xsl:value-of select="/DealerList/ZoneDescription" disable-output-escaping="yes"/>
                </div>
                <xsl:apply-templates select="/DealerList/DealerGroup"/>
                <div class="clear"/>
            </xsl:when>
            <xsl:otherwise>
                <div class="wrap-ketqua">
                    <xsl:if test="/DealerList/ResultText != ''">
                        <div class="title-room">
                            <xsl:value-of select="/DealerList/ResultText" disable-output-escaping="yes"/>
                        </div>
                    </xsl:if>
                    <table>
                        <xsl:apply-templates select="/DealerList/Dealer" mode="search"/>
                    </table>
                </div>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="DealerGroup">
        <div class="col-sm-6 PaddL0">
            ...
            <xsl:apply-templates select="Dealer" mode="load"/>
        </div>
    </xsl:template>
    <xsl:template match="Dealer" mode="load">
        <div class="item-diadiem">
            ...
            <xsl:if test="Phone != '' or Fax != ''">
                <br/>
                <xsl:if test="Phone != ''">
                    <xsl:value-of select="/DealerList/PhoneText" disable-output-escaping="yes"/>
                ...
                <xsl:if test="Fax != ''">
                    ...
                    <xsl:value-of select="/DealerList/FaxText" disable-output-escaping="yes"/>
            ...
            <td class="textright">
                <xsl:if test="Map != ''">
                    <a class="amaps fancybox.iframe">
                        ...
                        <xsl:value-of select="/DealerList/ViewMapText" disable-output-escaping="yes"/>
                    </a>
                </xsl:if>
            </td>
        </tr>
